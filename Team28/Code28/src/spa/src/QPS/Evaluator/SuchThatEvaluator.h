...
public:
    SuchThatEvaluator(QueryFacade *queryFacade)
        : queryFacade(queryFacade){};
    ClauseResult evaluate(SuchThatClause *suchThatCl);

private: 
    ClauseResult handleNoSynonym(RelationshipReference relRef, Reference left, Reference right);
    ClauseResult handleLeftSynonym(RelationshipReference relRef, Reference left, Reference right);
    ClauseResult handleRightSynonym(RelationshipReference relRef, Reference left, Reference right);
    ClauseResult handleBothSynonym(RelationshipReference relRef, Reference left, Reference right);
};
